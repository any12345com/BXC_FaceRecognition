...
int rtp_ext_playout_delay_parse(const uint8_t* data, int bytes, struct rtp_ext_playout_delay_t* ext)
{
    ...
    ext->min_delay = (((uint16_t)data[0]) << 4) | ((data[1] >> 4) & 0x0F);
    ext->max_delay = (((uint16_t)data[1] & 0x0F) << 8) | data[2];
    return 0;
}
...
int rtp_ext_playout_delay_write(uint8_t* data, int bytes, const struct rtp_ext_playout_delay_t* ext)
{
    ...
    data[0] = (uint8_t)(ext->min_delay >> 4) & 0xFF;
    data[1] = (uint8_t)(ext->min_delay & 0x0F) << 4;
    data[2] = (uint8_t)((ext->max_delay >> 8) & 0x0F);
    data[3] = (uint8_t)(ext->max_delay & 0xFF);
    return 3;
}
