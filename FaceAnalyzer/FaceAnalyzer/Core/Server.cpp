...
void api_controls(struct evhttp_request* req, void* arg) {
    ...
    if (len > 0) {
        ...
        for (int i = 0; i < controls.size(); i++)
        {
            ...
            result_data_item["pullApp"] = controls[i]->pullApp.data();
            result_data_item["pullName"] = controls[i]->pullName.data();
            result_data_item["isPushStream"] = controls[i]->isPushStream;
            result_data_item["pushApp"] = controls[i]->pushApp.data();
            result_data_item["pushName"] = controls[i]->pushName.data();
...
void api_control_add(struct evhttp_request* req, void* arg) {
    ...
    Scheduler* scheduler = (Scheduler*)arg;
    Config* config = scheduler->getConfig();
    ...
    if (reader->parse(buf, buf + std::strlen(buf), &root, &errs) && errs.empty()) {
        ...
        if (root["pullApp"].isString()) {
            control.pullApp = root["pullApp"].asCString();
        }
        if (root["pullName"].isString()) {
            control.pullName = root["pullName"].asCString();
        }
        if (root["isPushStream"].isInt()) {
            control.isPushStream = root["isPushStream"].asInt();
        }
        if (root["pushApp"].isString()) {
            control.pushApp = root["pushApp"].asCString();
        }
        if (root["pushName"].isString()) {
            control.pushName = root["pushName"].asCString();
        }
        if (!control.code.empty() && !control.pullApp.empty() && !control.pullName.empty()) {
            control.streamUrl = config->mediaRtspHost + "/" + control.pullApp + "/" + control.pullName;

            if (control.isPushStream) {
                //拼接推流地址
                control.pushStreamUrl = config->mediaRtspHost + "/" + control.pushApp + "/" + control.pushName;
            }
