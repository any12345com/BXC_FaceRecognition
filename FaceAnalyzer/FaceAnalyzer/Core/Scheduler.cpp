...
namespace BXC_Algorithm {
    ...
    bool Scheduler::initAlgorithm() {
        LOGI("开始初始化算法模型 %s,%s ...",
            mConfig->algorithmName.data(),
            mConfig->algorithmDevice.data());

        if ("model_v1.0" == mConfig->algorithmName) {
            mAlgorithm = new AlgorithmOpenCV(mConfig);
            LOGI("初始化算法模型成功");
        }else if ("model_v1.1" == mConfig->algorithmName) {
            mAlgorithm = new AlgorithmOpenVINO(mConfig);
            LOGI("初始化算法模型成功");
        }
        else {
            LOGE("初始化算法模型失败");
    ...
    Algorithm* Scheduler::gainAlgorithm() {
        ...
        if (mAlgorithmMtx.try_lock()) {
            if (mAlgorithm) {
                algorithm = mAlgorithm;
                mAlgorithm = nullptr;
            }
            mAlgorithmMtx.unlock();
        }
    ...
    void Scheduler::apiControlAdd(Control* control, int& result_code, std::string& result_msg) {
        ...
        if (getWorkerSize() >= mConfig->maxConcurrency) {
    ...
    bool Scheduler::addWorker(Control* control, Worker* worker) {
        ...
        mWorkerMapMtx.lock();
        if (mWorkerMap.size() < mConfig->maxConcurrency) {
            bool isAdd = mWorkerMap.end() != mWorkerMap.find(control->code);
            if (!isAdd) {
